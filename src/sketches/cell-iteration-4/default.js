...
function setup() {
  ...
  this.cells = [];
  this.cells.push(new Cell(createVector(0, 0, 0), createVector(CANVAS_WIDTH, CANVAS_HEIGHT, 0), color(0, 255)));
  this.cells[0].draw();
...
function * iterateCells() {
  for (let index = 1; index < ITERATIONS; index++) {
    ...
    for (let oldCellIndex = 0; oldCellIndex < oldCells.length; oldCellIndex++) {
      const newCells = iterate(oldCells[oldCellIndex], color((255.0/ITERATIONS)*index, 255/ITERATIONS));
