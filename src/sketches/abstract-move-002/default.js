...
function draw() {
  ...
  fill(random(this.colourScheme));
  const xPos = random(0, CANVAS_WIDTH);
  const yPos = random(0, CANVAS_HEIGHT);
  noStroke();
  ellipse(xPos, yPos, 10, 10);
  
  const position = createVector(xPos, yPos);
  const angle = atan2(CANVAS_HEIGHT/2 - position.y, CANVAS_WIDTH/2 - position.x) + Math.PI; // position.angleBetween(createVector(CANVAS_WIDTH/2, CANVAS_HEIGHT/2)); // + Math.PI;
  console.log(angle);
