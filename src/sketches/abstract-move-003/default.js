...
class BounceLines {
  ...
  draw() {
    ...
    for (let i = 0; i < this.lines.length-1; i++) {
      ...
      if (this.lines[i].outsideDrawZone && this.lines[i+1].outsideDrawZone) this.lines[i].toDelete = true;
    }
...
class BounceLine {
  ...
  update() {
    ...
    if (this.position.x < 0 || this.position.x > CANVAS_WIDTH) {
      if (this.position.y < 0 || this.position.y > CANVAS_HEIGHT) this.outsideDrawZone = true;
    }
  }
