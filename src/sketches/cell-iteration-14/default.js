    ...
    for (let z = 0; z < split; z++) {
      for (let y = 0; y < split; y++) {
        for (let x = 0; x < split; x++) {
          ...
          if (switchVal > 0 && switchVal < 0.33) newCell = new CellSphere3D(loc, newCellSize, nextColour);
          if (switchVal > 0.33 && switchVal < 0.66) newCell = new CellCone3D(loc, newCellSize, nextColour);
