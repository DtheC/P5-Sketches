...
function setup() {
  ...
  background(random(this.colourScheme));
  this.time = 0;
  this.cells = [];
...
function draw() {
  ...
  // white.setAlpha(10);
  // this.bg.setAlpha(10);
  // setGradient(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT, this.bg, white, 1);
  noStroke();
  this.cells.forEach(x => x.draw());
  this.cells = this.cells.filter(x => !x.toRemove);

  // Make New
  if (random() > 0.8 && frameCount < 100) {
    ...
    this.cells.sort((a, b) => b.scale-a.scale);
  }
  // this.bg.setAlpha(30);
  // background(this.bg);
  // this.cells.forEach(x => x.draw());
