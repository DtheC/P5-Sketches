...
function setup() {
  ...
  for (let x = 0; x < 20; x++) {
    for (let y = 0; y < 20; y++) {
      for (let z = 0; z < 20; z++) {
        rotateX(random(Math.PI/10));
        rotateY(random(Math.PI/10));
        rotateZ(random(Math.PI/10));
        const c = new Cell3D(createVector(x * 20, y * 20, z * 20), createVector(20, 20, 20), this.colourScheme[Math.floor(random(0, this.colourScheme.length))]);
        c.draw();
      }
    }
  }
